...
public class TwoPiecesChicken implements Meal{
    private static final int DRUMS_PER_ORDER = 1;
    ...
    public TwoPiecesChicken(String meal) {
        this.name = meal;
        this.type = CHICKEN;
    ...
    @Override
    public void count(
            Map<ChickenType, Integer> chickens) {
        if(!this.isSpicy) {
            if(this.isFried) {
                // regular and fried
